...
>
> This documentation is also available on my blog [here](http://vmorneau.me/apex-pwa-part1).

## APEX as a PWA: The Complete Guide

* **Part 1: Introducing PWAs**
* [Part 2: Pre-requisites and APEX Setup](./part2.md)
* [Part 3: Installing an APEX app into a mobile device](./part3.md)
* [Part 4: Using an APEX App Offline](./part4.md)
* [Part 5: Handling Background Sync](./part5.md)
* [Part 6: Sending Push Notifications](./part6.md)
* [Part 7: Tips When Developing a PWA](./part7.md)

## Part 1: Table of Content

* [Introducing PWAs](#introducing-pwas)
...
Let’s establish something clear right from the start: PWA stands for Progressive Web App and it refers to a bucket of concepts, aiming to close the gaps between traditionnal web apps and native apps.

PWAs are not about _one_ technology, they're about a set of modern browser APIs that, put together, make the case for a very compelling web user experience that ressembles a native app.
...
But there is no denial that native apps have clear benfits over web apps.
...
A native app can still work offline by offering partial functionnality and will synchronize pending requests with the server when the connectivity comes back.
...
[Part 3](./part3.md) of this guide explains how to install APEX on any device.
...
The browser provides a Cache API for storing ressources. In the context of an APEX application, our app shell are going to be APEX pages, along with the necessary JavaScript, CSS and other files to allow browsing while offline.

[Part 4](./part4.md) of this guide explains how to enable offline usage of an APEX application.
...
[Part 5](./part5.md) of this guide explains how to properly synchronize data when toggling between offline mode and online mode.

### Notification API

Browsers come with an API that allows us to handle push notifications. A push notification can be received at any time, wheter the user is browsing the app or not. The mobile device can be sleeping in a user's pocket and still received a web push notification.
...
A true PWA covers more, but this guide will not cover the following concepts, because I think those topics can be better learned elsewhere.
...
1. **A PWA is more reliable.**
   ...
   Another common use case is being on intermitent network, like using an application on the subway. We might loose the signal for a minute or two, and it becomes very convenient if we are not interrupted. A PWA allows that kind of reliability.

2. **A PWA is faster than a traditional web app**

   By leveraging the Cache API, PWAs can serve cached ressources (APEX Pages) to the user, which is much faster than reaching to the server. When a user requests a ressource that isn't cached already, the PWA will reach for the server and then cache the ressource for later.
...
3. **A PWA is more engaging**
   ...
   It will always be more convinient to reach for an icon on the home screen rather than bookmarking a URL. By having an APEX app explicitely sitting on a mobile device home screen, the user engagement level is significantly increased.
...
_Think this documentation can be better? Please open a pull request and fix it!_
